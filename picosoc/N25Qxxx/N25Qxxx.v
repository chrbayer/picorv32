...
reg resetDuringBusy=0;     //entity to check tRHSL timing constraint
    ...
    always @reset_by_powerOn if (reset_by_powerOn) begin : PON_reset
        ...
        if(prog.operation=="Subsector Erase" || prog.operation=="Subsector Erase 32K") begin
...
module CUIdecoder_TDP (cmdAllowed);
    ...
    always @(posedge N25Qxxx.S) if(N25Qxxx.latchingMode=="3" && attr[7:6] == 'b01) begin
        if(iData>129) begin
            $display("[ERROR] TDP program aborted, number of clock cycles %0d exceeds 128\n", iData);
        end else begin
            for(i=0;i<=63;i=i+1)begin
                tdp_reg[i] = tdp_reg_temp[i];
            end
            tdp_counter = 0;
            iData = 0;
        end
        ...
        //else if(prog.SwResetDuringSSE == 1) $display("[%0t ns] %0s PES after SW Reset is rejected IV", $time, operation);
            ...
            begin : exe
               ...
               @(posedge N25Qxxx.S);
                ...
                operation = N25Qxxx.cmdRecName;
                N25Qxxx.latchingMode="N";
                if(oldOperation == "Subsector Erase 32K" || oldOperation == "Subsector Erase" || oldOperation == "Sector Erase") begin
                      flag.FSR[6] = 1;
    ...
    function setMemory;
      ...
      begin
        case(current_die_active)
            'b00: N25Q_die0.mem.memory[memAddr] = memData;
...
`include "include/StackDecoder.h"
    ...
    function setMemory;
      ...
      begin
        case(current_die_active)
            'b00: MT25Q_die0.mem.memory[memAddr] = memData;
